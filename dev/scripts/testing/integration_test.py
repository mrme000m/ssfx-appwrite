#!/usr/bin/env python3
"""
dev/scripts/integration-test.py — Full-stack integration test for remote-services.

Builds the Docker image, starts all services, runs health + API + WS checks,
prints a report, and optionally tears down the stack.

Usage:
    ./dev.sh integration-test [--build] [--keep] [--logs]
    # Or directly:
    cd remote-services && python3 integration_test.py [--build] [--keep] [--logs]
"""
...
PROJECT_ROOT = Path(__file__).parent.parent.parent / "remote-services"
...
# Endpoints that require backend services (MongoDB, Appwrite, cTrader) and are
# expected to return 401/404/503 in a local test environment.
